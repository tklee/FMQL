...
function makeNavMarkup(useURIForm, base, reply)
{
    ...
    if (args["OFFSET"] != "0")
    {
        ...
        navMarkup += "<a href='" + prevURI + "'>PREV</a>";
    }
    else
        offset = 0;
    ...
    if (args["LIMIT"] == reply.results.length.toString())
    {
        ...
        navMarkup += "<a href='" + nextURI + "'>NEXT</a>";
    }
    if (navMarkup)
    {
...
function describeTypeResultToHTML(reply, useURIForm, base)
{
    ...
    fields = ["number", "name", "location", "type", "details", "index"];
    fieldNames = ["#", "Name", "Location", "Type", "Details", "Index"];
    ...
    for (var i = 0;i<results.length;i++)
    {
        ...
        for (var j = 0; j<fields.length; j++)
        {
            if (fields[j] in results[i])
            {
                ...
                if ((fields[j] == "name") && (results[i]["flags"].search(/R/) != -1))
